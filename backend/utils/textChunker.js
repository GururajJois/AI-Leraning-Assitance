        ...
        for (const paragraph of paragraphs) {
            ...
            if (paragraphWordCount > chunkSize) {
                ...
                for (let i = 0; i < paragraphWords.length; i += (chunkSize - overlap)) {
                    ...
                    chunks.push({ content: chunkWords.join(' '), 
                        chunkIndex: chunkIndex++,
                        pageNumber: 0 
                    });
 ...
 * @returns {Array<Object>} - The array of relevant chunks with relevance scores
 */
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
    ...
    const stopWords = new Set([
        'the','is','at', 'which', 'on', 'and', 'a', 'an', 'or','but','in','with','to','for','of','by','as','that','this','it'
    ]);
    ...
    const scoredChunks = chunks.map(chunk, index => {
        ...
        for(const word of queryWords) {
            //Exact word match (higher scrore)
            const excatMatches = (content.match(new RegExp(`\\b${word}\\b`, 'g')) || []).length;
            score += excatMatches * 3;

            //Partial word match (lower score)
            const partialMatches = (content.match(new RegExp(word, 'g')) || []).length;
            score += Math.max(0, partialMatches - excatMatches) * 1.5;
        }
        ...
        const postionBonus = 1 - (index / chunks.length) * 0.2;
        ...
        return {
            ...
            pageNumber: chunk.pageNumber,
            _id: chunk._id,
            score: normalizedScore * postionBonus,
            rawScore: score,
